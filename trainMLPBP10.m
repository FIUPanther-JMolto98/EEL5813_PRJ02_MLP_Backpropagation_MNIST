% trainMLPBP10.m - Function to train MLP classifier using BP for the number '3'
% this time, using a validation set from the first half of RSV_MNIST.mat
%
% SYNTAX: [W1,W2,b1,b2] = trainMLPBP10(alpha, epochs);
%
function [W1,W2,b1,b2] = trainMLPBP10(alpha, epochs)
    ...
    load('TRN_MNIST.mat', 'TRN10Y');
    TT_TRN = TRN10Y; % we assign the targets to a variable TT_TRN (1 x 5000)
    disp(['Size of TT_TRN (JUST LOADED FROM TRN_MNIST.mat FILE, TRN10Y MATRIX): ', mat2str(size(TT_TRN))]);
    % LOAD THE VALIDATION DATA VALXX
    load('RSV_MNIST_SPLIT_P2','VALXX');
    PP_VLD = VALXX;
    disp(['Size of PP_VLD (JUST LOADED): ', mat2str(size(PP_VLD))]);
    % LOAD THE VALIDATION TARGETS VAL10Y
    load('RSV_MNIST_SPLIT_P3.mat', 'VAL10Y');
    TT_VLD = VAL10Y;
    ...
    W2 = randn(10, NUM_HIDDEN_NEURONS) / 6; % random Gaussian Distribution matrix for small values W1 (Weight Matrix for LAYER 1)
    b1 = randn(NUM_HIDDEN_NEURONS, 1) / 6; % bias vector for the hidden layer, one for each processing element/neuron
    b2 = randn(10 , 1) / 6; % bias for the output layer
