% predictMLPBP1.m - Function to predict MLP for 1-DETECTOR using RSV data
%
% SYNTAX: [overallAcc, numCorrect, numIncorrect, MSE_Array] = predictMLPBP1(W1, W2, b1, b2, thr);
%
function [overallAcc, numCorrect, numIncorrect, MSE_Array] = predictMLPBP1(W1, W2, b1, b2, thr)
    
    % LOAD THE TESTING DATA FROM RSV_MNIST.mat, RSVXX VARIABLE
    load('RSV_MNIST.mat','RSVXX');
    PP = RSVXX; % we assign the training data to a variable PP (784 x 10000)
    % LOAD THE HOT-ONE ENCODED TARGETS FROM RSV_MNIST.mat, RSVY1 VARIABLE
    load('RSV_MNIST.mat', 'RSVY1');
    TT = RSVY1; % we assign the targets to a variable TT (1 x 100000)
    ...
    save('predictMLPBP1_results.mat', 'classificationResults', 'MSE_Array', 'numCorrect', 'numIncorrect', 'overallAcc');
end